...
<h4>1. 功能概述</h4>
<p>
本功能从 CATIA SPA（形状与尺寸分析）"测量惯量 + 保持测量" 写入的 Keep 参数中，读取每个零件的
...
<ol>
  ...
  <li>执行菜单 <b>形状 → 测量惯量</b>（Measure Inertia）。</li>
  <li>在对话框中勾选 <b>"保持测量"（Keep Measure）</b>，点击确定。</li>
  <li>CATIA 将在参数树中生成名为 <code>惯量包络体.1</code>（Inertia Keep.1）的几何体，
      其下包含以下参数：<br/>
      <code>质量</code>（kg）、<code>Gx / Gy / Gz</code>（重心坐标，mm）、
      <code>IoxG / IoyG / IozG / IxyG / IxzG / IyzG</code>（惯量分量，kg·m²）、
      以及可选的 <code>密度</code>（kg/m³）。</li>
  <li>如需为同一零件记录多次测量（例如不同材料区域），可重复以上步骤，
      CATIA 会依次生成 <code>惯量包络体.2</code>、<code>惯量包络体.3</code>……
      本功能最多读取编号 1 到 20 的保持测量。</li>
</ol>
<p>
<b>提示：</b>若产品树中存在已设置材料（Material）的零件，CATIA 会自动维护零件的质量和惯量，
此时只需执行一次"保持测量"即可获得最新值。若零件未设置材料，需在手动为零件赋密度后再执行测量。
</p>
...
<tr><td><b>汇总BOM</b></td>
    ...
    可选择排序列（零件编号、术语、重量等）对行进行排序。</td></tr>
</table>
...
<p><b>5.3 右键上下文菜单</b></p>
<p>右键单击表格中的任意行，可弹出如下菜单：</p>
<table border="0" cellpadding="4">
<tr><td><b>打开路径</b></td>
    <td>在文件资源管理器中打开该文件所在的文件夹。</td></tr>
<tr><td><b>复制路径</b></td>
    <td>将该行对应文件的完整路径复制到剪贴板。</td></tr>
<tr><td><b>在CATIA中打开</b></td>
    <td>通过系统关联程序在 CATIA 中打开该零件/产品文件。</td></tr>
<tr><td><b>重新读取质量特性</b></td>
    <td>（仅零件行可用）在不重新遍历产品树的情况下，从 CATIA 中重新读取该零件的
    惯量包络体 Keep 测量参数，并同步更新所有相同零件编号的节点以及汇总结果。
    适用于在 CATIA 中更新了零件测量后快速刷新单个零件的场景。</td></tr>
<tr><td><b>参与计算：√ / ×</b></td>
    <td>（层级BOM模式）切换该行是否参与质量汇总计算。排除后该行以灰色斜体显示；
    同一零件编号的所有实例同步排除/恢复。排除行不计入底部汇总，也不出现在汇总BOM中。</td></tr>
<tr><td><b>从列表删除</b></td>
    <td>（层级BOM模式）将该行从显示列表中移除（不影响 CATIA 文件）。</td></tr>
</table>

<p><b>5.4 展开 / 折叠 / 自适应列宽</b></p>
<p>底部按钮区提供"全部展开"、"全部折叠"、"自适应列宽"按钮，方便浏览大型装配树。
列宽可手动拖拽调整，切换模式后保留已调整的列宽。</p>

<h4>6. 汇总结果面板</h4>
<p>表格下方的"汇总结果（基于根产品坐标系）"面板，展示当前产品树中
所有<b>未被排除</b>的有效零件，在根产品坐标系下汇总后的结果：</p>
<ul>
  <li><b>总重量</b>：以当前所选重量单位（g 或 kg）显示。</li>
  <li><b>总重心 X / Y / Z</b>：以当前所选长度单位（mm 或 m）显示。</li>
  <li><b>Ixx / Iyy / Izz / Ixy / Ixz / Iyz</b>：以当前所选惯量单位显示。</li>
</ul>
<p>每次修改重量/密度或切换排除状态后，该面板自动刷新。</p>
