...
<table border="0" cellpadding="4">
  ...
  <tr>
    ...
    <td>
      ...
      <i>需为每个零件预先通过 CATIA "测量惯量 + 保持测量" 功能生成
        <code>惯量包络体.x</code> 参数组。详见下方专项说明。</i>
    </td>
  </tr>
  <tr>
    <td><b>新建图纸</b></td>
    <td>
      ...
      CATProduct 生成新图纸。<br />
      <i>需在 CATIA 中打开目标零件/装配体，并将 *.CATDrawing 模板放入 drawing_templates 文件夹。</i>
    </td>
  </tr>
  <tr>
    <td><b>刷新图纸</b></td>
    <td>
      ...
      <i>需在 CATIA 中同时打开目标图纸和对应零件/装配体文档。</i>
    </td>
  </tr>
</table>
...
<table border="0" cellpadding="4">
  ...
  <tr>
    <td><b>增加对称件</b></td>
    <td>
      （层级BOM模式，仅单选）为选中行新增一条虚拟的"对称件"行，位于选中行正上方，以浅蓝色背景标识。
  ...
  <tr>
    <td><b>参与计算：√ / ×</b></td>
    <td>
      （层级BOM模式）切换该行是否参与质量汇总计算。排除后该行以灰色斜体显示；
  ...
  <tr>
    <td><b>从列表删除</b></td>
    <td>
      （层级BOM模式）将该行及其全部子孙行从显示列表中移除（不影响 CATIA 文件）。
...
<p>
  在层级BOM模式下，右键选中任意行（零件、产品或部件），点击 <b>"增加对称件"</b>，
...
<table border="0" cellpadding="4">
  <tr>
    <td><b>— （破折号）</b></td>
    <td>
      该零件未执行"保持测量"，或 CATIA 参数树中找不到对应的 <code>惯量包络体.x</code>。 请在 CATIA
      中<b>单独打开</b>该零件，执行测量后重新加载或使用右键"重新读取质量特性"。
    </td>
  </tr>
  <tr>
    <td><b>文件未找到</b></td>
...
<table border="0" cellpadding="4">
  ...
  <tr>
    ...
    <td>
      A: 这表示该零件未完成"保持测量"，或测量结果无法被读取。 可能原因：<br />
