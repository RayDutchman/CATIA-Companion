...
<tr><td><b>重量、重心、惯量统计</b></td>
    <td>遍历当前 CATProduct 的产品树，读取每个零件的质量（重量）、重心坐标及转动惯量，
    在根产品坐标系下按装配层级自动汇总，并支持导出至 Excel。<br/>
    <i>需为每个零件预先通过 CATIA SPA "测量惯量 + 保持测量" 功能生成
    <code>惯量包络体.x</code> 参数组。详见下方专项说明。</i></td></tr>
<tr><td><b>新建图纸</b></td>
...
<h4>1. 功能概述</h4>
<p>
本功能从 CATIA 测量工具栏的 "测量惯量 + 保持测量" 写入的保持测量参数中，读取每个零件的
质量（kg）、重心坐标（mm）和转动惯量张量（kg·m²），并将所有数据统一变换到<b>根产品坐标系</b>，
再按装配层级逐级汇总，最终展示整棵产品树的完整质量特性，并可导出至 Excel。
</p>
<p>
所有内部计算均使用<b>国际单位制（SI）</b>：质量 kg、坐标 m、转动惯量 kg·m²。
界面显示和导出时根据用户选择的单位换算为实用单位（g/kg、mm/m、g·mm²/g·m²/kg·mm²/kg·m²）。
</p>

<h4>2. CATIA 前提条件：惯量保持测量</h4>
<p>在使用本功能前，必须为产品树中<b>每个需要统计的零件</b>完成以下操作：</p>
<ol>
  <li><b>单独打开零件文件</b>（CATPart），不要在产品窗口中操作。<br/>
      <i>原因：在产品窗口下建立的测量，其参考坐标系为根产品坐标系，而非零件自身坐标系。
      本功能期望重心坐标在零件局部坐标系下给出，再由软件统一变换到根产品坐标系；
      若测量时已采用根产品坐标系，坐标系不重合的零件计算结果将出错。</i></li>
  <li>执行菜单 <b>测量 → 测量惯量</b>（Measure Inertia）。</li>
  <li>在对话框中勾选 <b>"保持测量"（Keep Measure）</b>，点击确定。</li>
  <li>CATIA 将在参数树中生成名为 <code>惯量包络体.1</code>的几何体，
      其下包含以下参数：<br/>
      <code>质量</code>（kg）、<code>密度</code>（kg/m³）、<code>Gx / Gy / Gz</code>（重心坐标，mm）、
      <code>IoxG / IoyG / IozG / IxyG / IxzG / IyzG</code>（惯量分量，kg·m²）</li>
  <li>如需为同一零件记录多次测量（例如不同材料区域），可重复以上步骤，
...
<table border="0" cellpadding="4">
<tr><td><b>从 CATIA 现场加载</b></td>
    <td>
    <ul>
      <li>勾选 <b>"使用当前 CATIA 活动文档"</b>：直接读取 CATIA 中当前处于激活状态的 CATProduct。</li>
      <li>或点击 <b>"浏览…"</b> 选择磁盘上的 CATProduct 文件，再点击 <b>"加载"</b>。
          CATIA 会自动打开该文件（如尚未打开）后进行遍历。</li>
    </ul>
    加载过程中会显示进度提示，对于大型装配体可能需要数分钟。
    </td></tr>
<tr><td><b>从已保存数据载入</b></td>
    <td>点击 <b>"载入已保存数据…"</b>，选择之前通过"保存数据"按钮保存的 <code>.mpd</code> 数据文件。
    这种方式无需打开 CATIA，适合离线分析或数据共享。</td></tr>
<tr><td><b>追加数据… / 追加活动文档…</b></td>
    <td>在已加载基础产品数据后，可将额外的分总成质量特性追加合并，适用于主产品过大、需分批采集各分总成的场景。
    <ul>
      <li><b>追加数据…</b>：从一个或多个 <code>.mpd</code> 数据文件中追加分总成质量特性（支持多文件同时选取）。</li>
      <li><b>追加活动文档…</b>：将 CATIA 当前活动文档（分总成 CATProduct）的质量特性实时追加到现有数据中。</li>
    </ul>
    <b>前提：各分总成的坐标系须与主产品（及彼此）一致，程序不执行额外坐标变换。</b><br/>
    两个按钮在加载基础数据后方才启用；若当前尚无数据，操作会被提示并阻止。</td></tr>
</table>

<h4>4. 读取与显示选项</h4>

<p><b>4.1 惯量包络体读取模式</b>（对加载性能有影响）</p>
<table border="0" cellpadding="4">
<tr><td><b>只读.1</b></td>
    <td>仅读取每个零件的 <code>惯量包络体.1</code>。速度最快：每个零件只进行一次参数查询。
    适用于每个零件只有一个保持测量的常规情况。</td></tr>
<tr><td><b>最大编号</b></td>
    <td>扫描编号 1 到 20 的全部惯量包络体，取编号最大的有效保持测量结果。
    适用于需要以"最新一次测量"为准的场景（例如重复测量后保留最后一次）。
    速度较慢：每个不存在的编号均会产生一次 COM 异常，最多产生 19 次。</td></tr>
<tr><td><b>全部汇总</b>（默认）</td>
    <td>读取全部有效编号（1–20），按<b>平行轴定理</b>在零件级汇总为单一质量特性。
    适用于一个零件有多个分区域测量（如多材料零件各区域分别建立惯量包络体）的场景。
    速度与"最大编号"模式相同。</td></tr>
</table>

<p><b>4.2 BOM 展示模式</b></p>
<table border="0" cellpadding="4">
<tr><td><b>层级BOM</b>（默认）</td>
    <td>以树形结构展示完整产品树，包含零件（叶节点）和产品/部件（中间节点）。
    产品/部件行显示其子树内所有零件的<b>汇总值</b>（在根产品坐标系下）。
    零件行的 Weight / CogX/Y/Z / Ixx–Iyz 均已变换到<b>根产品坐标系</b>，与装配位置有关。</td></tr>
<tr><td><b>汇总BOM</b></td>
    <td>按零件编号合并相同零件，每个唯一零件编号（Part Number）显示为一行，
    并显示数量（Quantity）。仅列出零件（不含产品和部件节点）。
    Weight / CogX/Y/Z / Ixx–Iyz 在<b>零件自身坐标系</b>下显示，与装配位置无关。
    可选择排序列（零件编号、术语、版本、文件名、数量、重量等）对行进行排序。</td></tr>
</table>

<p><b>4.3 单位设置</b></p>
<table border="0" cellpadding="4">
<tr><td><b>重量单位</b></td>
    <td>g 或 kg（默认 g）。</td></tr>
<tr><td><b>长度单位（重心坐标）</b></td>
    <td>mm 或 m（默认 mm）。</td></tr>
<tr><td><b>惯量单位</b></td>
    <td>g·mm²、g·m²、kg·mm² 或 kg·m²（默认 g·mm²）。惯量单位独立于重量和长度单位单独选择。</td></tr>
</table>
...
<p><b>4.5 忽略隐藏的节点</b></p>
<p>勾选后：零件处于隐藏状态（不可见）则跳过；产品/部件处于隐藏状态则连同其子孙一并跳过，
不纳入统计。</p>

<h4>5. 表格操作</h4>

<p>表格支持<b>多选</b>（Ctrl＋单击 或 Shift＋单击）。多选时通过右键上下文菜单可对选中的所有行
执行批量操作（批量删除、批量切换参与计算、批量重新读取）。若右键单击的行不在当前选中集中，
程序会清除原有选中并仅选中该行（退化为单选）。</p>

<p><b>5.1 编辑重量</b></p>
<p>双击零件行的"重量"单元格可直接输入修改值（仅零件行可编辑，产品/部件汇总行不可编辑）。
修改后：</p>
<ul>
  <li>按相同比例等比缩放该零件的全部惯量分量（保持质量-惯量关系一致）。</li>
  <li>同步更新所有相同零件编号（Part Number）的其他行。</li>
  <li>自动重新计算装配体总质量特性，底部"汇总结果"面板实时刷新。</li>
  <li>重量修改同时等比更新密度（体积不变，密度 = 质量 / 体积）。</li>
</ul>
<p>输入 0 表示将该零件重量设为 0；不允许输入负数。</p>

<p><b>5.2 编辑密度</b></p>
<p>双击零件行的"密度"单元格可直接修改密度值（kg/m³）。修改后按比例缩放该零件的重量和惯量
（体积不变，质量 = 密度 × 体积）。以下情况密度列不可编辑：</p>
<ul>
...
<table border="0" cellpadding="4">
<tr><td><b>打开路径</b></td>
    <td>（仅单选）在文件资源管理器中打开该文件所在的文件夹。</td></tr>
<tr><td><b>复制路径</b></td>
    <td>（仅单选）将该行对应文件的完整路径复制到剪贴板。</td></tr>
<tr><td><b>在CATIA中打开</b></td>
    <td>（仅单选）通过系统关联程序在 CATIA 中打开该零件/产品文件。</td></tr>
<tr><td><b>重新读取质量特性</b></td>
    <td>（仅零件行可用）在不重新遍历产品树的情况下，从 CATIA 中重新读取该零件的
    惯量包络体保持测量参数，并同步更新所有相同零件编号的节点以及汇总结果。
    适用于在 CATIA 中更新了零件测量后快速刷新单个零件的场景。多选时对所有选中的零件行批量执行。</td></tr>
<tr><td><b>增加对称件</b></td>
    <td>（层级BOM模式，仅单选）为选中行新增一条虚拟的"对称件"行，位于选中行正下方，以浅蓝色背景标识。
    对称规则：相对 ZX 平面对称（即 CogY、Ixy、Iyz 取反，其余分量不变）。
    对称件行参与汇总计算，但不可直接编辑质量/密度；当源行被修改或删除时，对称件行自动同步或级联删除。
    详见第 5.5 节。</td></tr>
<tr><td><b>参与计算：√ / ×</b></td>
    <td>（层级BOM模式）切换该行是否参与质量汇总计算。排除后该行以灰色斜体显示；
    同一零件编号的所有实例同步排除/恢复。排除行不计入底部汇总，也不出现在汇总BOM中。
    多选时：若所有选中行均已排除，则全部恢复参与；否则将全部行标记为排除。</td></tr>
<tr><td><b>从列表删除</b></td>
    <td>（层级BOM模式）将该行及其全部子孙行从显示列表中移除（不影响 CATIA 文件）。
    删除前会弹出确认对话框以防误操作。若该行有关联对称件，对称件行同时级联删除。
    多选时对所有选中行及其子树批量执行。</td></tr>
</table>

<p><b>5.4 展开 / 折叠 / 自适应列宽</b></p>
<p>底部按钮区提供"全部展开"、"全部折叠"、"自适应列宽"按钮，方便浏览大型装配树。
列宽可手动拖拽调整，切换模式后保留已调整的列宽。
重新加载或追加数据后，程序自动恢复加载前的节点展开/折叠状态。</p>

<p><b>5.5 对称件（虚拟行）功能</b></p>
<p>在层级BOM模式下，右键选中任意行（零件、产品或部件），点击 <b>"增加对称件"</b>，
即可在该行正下方插入一条<b>虚拟对称件行</b>。对称件行以<b>浅蓝色背景</b>显示，类型列显示"对称件"。</p>
...
<h4>6. 汇总结果面板</h4>
<p>表格下方的"汇总结果（基于根产品坐标系）"面板，展示当前产品树中
所有<b>未被排除</b>的有效零件，在根产品坐标系下汇总后的结果。面板分为三列：</p>
<table border="0" cellpadding="4">
<tr><td><b>左列：总重量 + 重心 (G)</b></td>
    <td>
      <ul>
...
<tr><td><b>中列：惯量矩阵</b></td>
    <td>以 3×3 完整矩阵形式展示关于重心的转动惯量张量：<br/>
      Ixx、Ixy、Ixz<br/>
      Iyx、Iyy、Iyz<br/>
      Izx、Izy、Izz<br/>
      （以当前所选惯量单位显示；矩阵对称，Ixy=Iyx 等）</td></tr>
<tr><td><b>右列：重心主惯量矩 + 主轴</b></td>
    <td>对惯量矩阵进行特征值分解（Jacobi 方法），给出：
      <ul>
        <li><b>重心主惯量矩 M1 / M2 / M3</b>：三个主惯量矩（按升序排列，以当前惯量单位显示）。</li>
        <li><b>主轴 A1 / A2 / A3</b>：对应主惯量矩的单位方向向量（各给出 x / y / z 分量，无量纲）。</li>
      </ul>
    </td></tr>
</table>
...
<table border="0" cellpadding="4">
<tr><td><b>导出表格</b></td>
    <td>将当前表格（含底部汇总行）导出为 Excel（.xlsx）文件。
    导出内容：所有当前可见列（不含内部序号列 #），末尾追加 Status（状态）列。<br/>
    排除行以特殊背景色标记；测量失败的零件行以橙色标记；
    数值均以当前所选单位导出。</td></tr>
<tr><td><b>保存数据…</b></td>
    <td>将当前行数据保存为 <code>.mpd</code> 压缩二进制文件（内容为 gzip 压缩的 JSON）。
    保存后可随时通过"载入已保存数据…"在不连接 CATIA 的情况下重新打开。</td></tr>
</table>

<h4>8. 常见错误状态说明</h4>
<table border="0" cellpadding="4">
<tr><td><b>— （破折号）</b></td>
    <td>该零件未执行"保持测量"，或 CATIA 参数树中找不到对应的 <code>惯量包络体.x</code>。
    请在 CATIA 中<b>单独打开</b>该零件，执行测量后重新加载或使用右键"重新读取质量特性"。</td></tr>
<tr><td><b>文件未找到</b></td>
    <td>磁盘上找不到该零件文件，可能已被移动或重命名。无法进行质量特性读取。</td></tr>
<tr><td><b>密度显示 —</b></td>
    <td>CATIA 测量时未读取到密度参数（零件未设置材料），该单元格不可编辑。</td></tr>
<tr><td><b>密度显示 -1</b></td>
    <td>CATIA 报告该零件材料不统一（多材料或材料设置冲突），无法给出单一密度，该单元格不可编辑。</td></tr>
</table>

<h4>9. 技术背景：坐标变换与惯量汇总算法</h4>
<p>本功能内部采用以下算法：</p>
<ul>
  <li><b>重心坐标变换</b>（局部 → 根坐标系）：<br/>
      r_根 = R × r_局部 + T<br/>
      其中 R 为零件相对根产品的旋转矩阵，T 为平移向量，均由 CATIA Position.GetComponents() 读取。</li>
  <li><b>转动惯量旋转变换</b>（局部 → 根坐标系）：<br/>
      I_根 = R × I_局部 × R^T</li>
  <li><b>装配级汇总（平行轴定理）</b>：对于一个装配节点，将各子零件的重心处惯量
      先移至根坐标原点再汇总，最后用平行轴定理移回总重心：<br/>
      I_总重心 = Σ(I_i) + Σ m_i (|r_i|²E - r_i ⊗ r_i) − M (|r_c|²E - r_c ⊗ r_c)</li>
  <li><b>零件级多惯量包络体汇总</b>：当一个零件有多个保持测量时（"全部汇总"模式），
      同样用上述平行轴定理在零件局部坐标系下合并，得到单一等效质量特性，再执行装配级变换。</li>
</ul>
...
<tr><td><b>Q: 质量特性统计为何某些零件显示 "—"（破折号）？</b></td>
    <td>A: 这表示该零件未完成"保持测量"，或测量结果无法被读取。
    可能原因：<br/>
    ① 零件未在 CATIA 中执行过"形状 → 测量惯量 → 保持测量"；<br/>
    ② 测量是在产品窗口下建立的（而非单独打开零件文件），导致参数命名前缀与预期不符；<br/>
    ③ 惯量包络体编号超过读取上限（默认最多读取编号 1–20）。<br/>
    解决方法：<b>单独打开</b>该零件文件，执行测量后重新加载，或右键点击该行选择"重新读取质量特性"。</td></tr>
</table>
